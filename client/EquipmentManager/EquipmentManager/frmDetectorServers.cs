...
// Type: CascadeEquipment.FrmDetectorServers
...
{
  public class FrmDetectorServers : XtraForm
  {
    private DataTable _dtMain = new DataTable();
    public bool ListFlag = false;
    public BcDetectorServer MainServer = new BcDetectorServer();
    ...
    public void CheckServices()
    {
      while (!FrmDevices.ClosFlag)
      {
        try
        {
          ...
          while (!FrmDevices.ClosFlag)
          {
            this.Invoke((Delegate) new FrmDetectorServers.RefreshStateFunc(this.RefreshState), (object) managmentServerClient.GetDetectorServerState());
            Thread.Sleep(5000);
          }
          try
          {
    ...
    private void frmDetectorServers_Load(object sender, EventArgs e)
    {
      ...
      this.ReloadGrid();
      this.tbNumber.EditValue = (object) (BcDetectorServer.LoadMaxNumber() + 1);
      this.tbPort.EditValue = (object) BcDetectorServer.LoadMaxPort();
    }
    ...
    private void btAdd_ItemClick(object sender, ItemClickEventArgs e)
    {
      ...
      this._dtMain.Rows.Add((object) Guid.Empty, (object) num1, (object) (Messages.DetectionServer + (object) num1), (object) "", (object) "0.0.0.0", (object) num2, (object) DateTime.Now, (object) DateTime.Now, (object) Messages.Unavailable, (object) true);
    }
    ...
    private void btSave_ItemClick(object sender, ItemClickEventArgs e)
    {
      ...
      foreach (DataRow dataRow in (InternalDataCollectionBase) this._dtMain.Rows)
      {
        ...
        {
          BcDetectorServer bcDetectorServer = new BcDetectorServer();
          bcDetectorServer.Id = (Guid) dataRow["ID"];
          bcDetectorServer.Ip = dataRow["IP"].ToString();
          bcDetectorServer.Port = (int) dataRow["Port"];
          bcDetectorServer.Name = dataRow["Name"].ToString();
          bcDetectorServer.Comment = dataRow["Comment"].ToString();
          bcDetectorServer.Number = (int) dataRow["Number"];
          bcDetectorServer.Save();
          dataRow["ID"] = (object) bcDetectorServer.Id;
    ...
    private void btAccept_Click(object sender, EventArgs e)
    {
      ...
      this.MainServer = new BcDetectorServer();
    ...
    {
      ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (FrmDetectorServers));
